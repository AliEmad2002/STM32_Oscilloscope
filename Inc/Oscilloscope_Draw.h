...
/*
 * fills a segment in selected line in not in use image buffer with any color
 */
#define FILL_SEGMENT(line, start, end, color)                              \
{                                                                          \
	DMA_voidSetMemoryAddress(                                              \
		DMA_UnitNumber_1, LINE_SEGMENT_DMA_CHANNEL,                        \
		(void*)&Global_ImgBufferArr[Global_NotInUseImgBufferIndex][(line)  \
		* SIGNAL_LINE_LENGTH + (start)]);								   \
                                                                           \
...
{                                                                          \
	for (                                                                  \
		u16 i = SIGNAL_LINE_LENGTH / 2;                                    \
		i < LINES_PER_IMAGE_BUFFER * SIGNAL_LINE_LENGTH;                   \
...
{                                                                          \
	...
	if (                                                                   \
		Global_IsCh1Enabled &&                                             \
		CONVERT_UV_TO_PIX_CH1(Global_Offset1MicroVolts) +                  \
		SIGNAL_LINE_LENGTH / 2 > OFFSET_POINTER_LEN              &&        \
		CONVERT_UV_TO_PIX_CH1(Global_Offset1MicroVolts) +                  \
		SIGNAL_LINE_LENGTH / 2 < SIGNAL_LINE_LENGTH - OFFSET_POINTER_LEN   \
	)                                                                      \
	{                                                                      \
		u8 start =                                                         \
			CONVERT_UV_TO_PIX_CH1(Global_Offset1MicroVolts) +              \
			SIGNAL_LINE_LENGTH / 2 - OFFSET_POINTER_LEN / 2;               \
		u8 n = OFFSET_POINTER_LEN;                                         \
		for (u8 j = 0; j < OFFSET_POINTER_LEN / 2 + 1; j++)                \
		{                                                                  \
			for (u8 k = start; k < start + n; k++)                         \
			{                                                              \
				Global_ImgBufferArr[Global_NotInUseImgBufferIndex]         \
				[SIGNAL_LINE_LENGTH * j + k] =                             \
...
{                                                                          \
	...
	if (                                                                   \
		Global_IsCh2Enabled &&                                             \
		CONVERT_UV_TO_PIX_CH2(Global_Offset2MicroVolts) +                  \
		SIGNAL_LINE_LENGTH / 2 > OFFSET_POINTER_LEN             &&         \
		CONVERT_UV_TO_PIX_CH2(Global_Offset2MicroVolts) +                  \
		SIGNAL_LINE_LENGTH / 2 < SIGNAL_LINE_LENGTH - OFFSET_POINTER_LEN   \
	)                                                                      \
